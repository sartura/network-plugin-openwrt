...
static int insert_sr_node(struct list_head *list, struct json_object *jobj,
  ...
  CHECK_NULL(jobj, &rc, cleanup, "failed json_object_object_get_ex for %s",
             j_name);
...
static void ubus_base_cb(struct ubus_request *req, int type,
...
static int ubus_base(const char *ubus_lookup_path, struct status_container *msg,
  ...
  u_rc = ubus_invoke(ctx, id, msg->ubus_method, blob->head, ubus_base_cb,
                     (void *)msg, UBUS_INVOKE_TIMEOUT);
...
static void sfp_rx_pwr_cb(struct json_object *obj, struct list_head *list) {
  int rc = SR_ERR_OK;
  const char *fmt = "/ietf-interfaces:interfaces-state/interface[name='wan']/"
